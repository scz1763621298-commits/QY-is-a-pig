...
export const Experience: React.FC<ExperienceProps> = ({ uploadedImages, onCameraReady, onError }) => {
    ...
    // Helper: Create Mesh
    const createPhotoMesh = (tex: THREE.Texture, w: number, h: number) => {
         ...
         if (newImages.length > 0) {
            newImages.forEach(imageUrl => {
                ...
                img.onload = () => {
                    ...
                    const mesh = createPhotoMesh(tex, w, h);
                ...
                img.onerror = () => {
                    ...
                    // Create Fallback "Missing" Texture
                    const canvas = document.createElement('canvas');
                    canvas.width = 256; canvas.height = 256;
                    const ctx = canvas.getContext('2d');
                    if(ctx) {
                        ctx.fillStyle = '#aa00aa'; // Magenta
                        ctx.fillRect(0,0,256,256);
                        ctx.fillStyle = 'white';
                        ctx.font = '30px Arial';
                        ctx.textAlign = 'center';
                        ctx.textBaseline = 'middle';
                        ctx.fillText('MISSING', 128, 100);
                        // Try to show filename
                        const filename = imageUrl.split('/').pop() || '?';
                        ctx.font = '20px Arial';
                        ctx.fillText(filename.substring(0, 15), 128, 150);
                    }
                    const tex = new THREE.CanvasTexture(canvas);
                    const mesh = createPhotoMesh(tex, 3, 3);
                    const event = new CustomEvent('add-photo', { detail: { mesh, url: imageUrl } });
                    window.dispatchEvent(event);
                }
